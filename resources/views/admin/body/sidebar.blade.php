<nav id="sidebar" 
    class="bg-white/80 dark:bg-gray-900/80 backdrop-blur-lg w-64 p-4 md:block hidden transition-all duration-300 ease-in-out border-r border-slate-200 dark:border-slate-800">
    ...
    <div id="nav-links" class="space-y-1">
        ...
        <a href="{{ route('dashboard') }}"
            ...
            {{ request()->routeIs('dashboard') ? 'bg-blue-500/10 text-blue-600 font-semibold dark:text-sky-300' : 'text-slate-600 hover:bg-slate-200/60 dark:text-slate-300 dark:hover:bg-slate-700/60' }}">
            @if(request()->routeIs('dashboard'))
                <span class="absolute inset-y-0 left-0 w-1 rounded-r-full bg-blue-500"></span>
            @endif
        ...
        @if (Auth::user()->can('category.menu'))
            <a href="{{ route('all.category') }}"
                class="relative nav-link flex items-center py-2.5 px-4 rounded-lg transition-colors duration-200
                {{ request()->routeIs('all.category') ? 'bg-blue-500/10 text-blue-600 font-semibold dark:text-sky-300' : 'text-slate-600 hover:bg-slate-200/60 dark:text-slate-300 dark:hover:bg-slate-700/60' }}">
        ...
        @if (Auth::user()->can('product.menu'))
            <a href="{{ route('all.product') }}"
                class="relative nav-link flex items-center py-2.5 px-4 rounded-lg transition-colors duration-200
                {{ request()->routeIs('all.product') ? 'bg-blue-500/10 text-blue-600 font-semibold dark:text-sky-300' : 'text-slate-600 hover:bg-slate-200/60 dark:text-slate-300 dark:hover:bg-slate-700/60' }}">
        ...
        @if (Auth::user()->can('customer.menu'))
             <a href="{{ route('customer.all') }}"
                class="relative nav-link flex items-center py-2.5 px-4 rounded-lg transition-colors duration-200
                {{ request()->routeIs('customer.all') ? 'bg-blue-500/10 text-blue-600 font-semibold dark:text-sky-300' : 'text-slate-600 hover:bg-slate-200/60 dark:text-slate-300 dark:hover:bg-slate-700/60' }}">
        ...
        @if (Auth::user()->can('supplier.menu'))
            <a href="{{ route('all.supplier') }}"
                class="relative nav-link flex items-center py-2.5 px-4 rounded-lg transition-colors duration-200
                {{ request()->routeIs('all.supplier') ? 'bg-blue-500/10 text-blue-600 font-semibold dark:text-sky-300' : 'text-slate-600 hover:bg-slate-200/60 dark:text-slate-300 dark:hover:bg-slate-700/60' }}">
        ...
        @if (Auth::user()->can('stock.menu'))
            <a href="{{ route('all.stock') }}"
                class="relative nav-link flex items-center py-2.5 px-4 rounded-lg transition-colors duration-200
                {{ request()->routeIs('all.stock') ? 'bg-blue-500/10 text-blue-600 font-semibold dark:text-sky-300' : 'text-slate-600 hover:bg-slate-200/60 dark:text-slate-300 dark:hover:bg-slate-700/60' }}">
            ...
            <div id="purchaseDropdown" class="relative group">
                @if ($purchaseMenu)
                    <a href="{{ route('pending.purchase') }}"
                        ...
                        {{ $isPurchaseActive ? 'bg-blue-500/10 text-blue-600 font-semibold dark:text-sky-300' : 'text-slate-600 hover:bg-slate-200/60 dark:text-slate-300 dark:hover:bg-slate-700/60' }}">
                        @if($isPurchaseActive)
                            <span class="absolute inset-y-0 left-0 w-1 rounded-r-full bg-blue-500"></span>
                        @endif
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
            ...
            <div id="expenseDropdown" class="relative group">
                @if ($canAdd)
                    <a href="{{ route('add.expense') }}"
                        class="relative nav-link flex items-center py-2.5 px-4 rounded-lg w-full transition-colors duration-200
                        {{ $isExpenseActive ? 'bg-blue-500/10 text-blue-600 font-semibold dark:text-sky-300' : 'text-slate-600 hover:bg-slate-200/60 dark:text-slate-300 dark:hover:bg-slate-700/60' }}">
                        @if($isExpenseActive)
                            <span class="absolute inset-y-0 left-0 w-1 rounded-r-full bg-blue-500"></span>
                        @endif
            ...
            <div id="orderDropdown" class="relative group">
                @if ($orderMenu)
                    <a href="{{ route('pending.order') }}"
                        class="relative nav-link flex items-center py-2.5 px-4 rounded-lg w-full transition-colors duration-200
                        {{ $isOrderActive ? 'bg-blue-500/10 text-blue-600 font-semibold dark:text-sky-300' : 'text-slate-600 hover:bg-slate-200/60 dark:text-slate-300 dark:hover:bg-slate-700/60' }}">
                        @if($isOrderActive)
                            <span class="absolute inset-y-0 left-0 w-1 rounded-r-full bg-blue-500"></span>
                        @endif
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
        ...
        @if (Auth::user()->can('role.menu'))
            ...
            <div id="permissionDropdown" class="relative group">
                <a href="{{ route('all.permission') }}"
                    class="relative nav-link flex items-center py-2.5 px-4 rounded-lg w-full transition-colors duration-200
                    {{ $isPermissionActive ? 'bg-blue-500/10 text-blue-600 font-semibold dark:text-sky-300' : 'text-slate-600 hover:bg-slate-200/60 dark:text-slate-300 dark:hover:bg-slate-700/60' }}">
        ...
        @if (Auth::user()->can('user.menu'))
            <a href="{{ route('all.admin') }}"
                class="relative nav-link flex items-center py-2.5 px-4 rounded-lg transition-colors duration-200
                {{ request()->routeIs('all.admin') ? 'bg-blue-500/10 text-blue-600 font-semibold dark:text-sky-300' : 'text-slate-600 hover:bg-slate-200/60 dark:text-slate-300 dark:hover:bg-slate-700/60' }}">
        ...
        @if (Auth::user()->can('backup.menu'))
            <a href="{{ route('admin.backup') }}"
                class="relative nav-link flex items-center py-2.5 px-4 rounded-lg transition-colors duration-200
                {{ request()->routeIs('admin.backup') ? 'bg-blue-500/10 text-blue-600 font-semibold dark:text-sky-300' : 'text-slate-600 hover:bg-slate-200/60 dark:text-slate-300 dark:hover:bg-slate-700/60' }}">
            ...
            <div id="reportDropdown" class="relative group">
                @if ($reportMenu)
                    <a href="{{ route('all.reports') }}"
                        class="relative nav-link flex items-center py-2.5 px-4 rounded-lg w-full transition-colors duration-200
                        {{ $isReportActive ? 'bg-blue-500/10 text-blue-600 font-semibold dark:text-sky-300' : 'text-slate-600 hover:bg-slate-200/60 dark:text-slate-300 dark:hover:bg-slate-700/60' }}">
                        @if($isReportActive)
                            <span class="absolute inset-y-0 left-0 w-1 rounded-r-full bg-blue-500"></span>
                        @endif
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
