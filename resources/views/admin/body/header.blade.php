<script>
    // Script សម្រាប់ពិនិត្យ Theme ដំបូងគេ
...
<div id="exchange-rate-modal" class="hidden fixed inset-0 z-50 overflow-y-auto bg-slate-900/50 backdrop-blur-sm transition-opacity duration-300">
    <div class="relative top-10 sm:top-20 mx-auto w-full max-w-sm transform rounded-xl bg-white p-6 shadow-2xl transition-all duration-300 dark:bg-slate-800 border dark:border-slate-700">
        ...
        <form id="exchangeRateForm" class="mt-4 text-left space-y-4">
            @csrf
            <div>
            ...
            <div class="pt-4 flex justify-end gap-x-3">
                ...
                <button id="save-exchange-rate-btn" type="submit" class="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 focus:outline-none">Save Rate</button>
            </div>
        </form>
    </div>
</div>
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // --- PART 3: Exchange Rate Modal Logic ---
    ...
    if (exchangeModal) {
        ...
        form.addEventListener('submit', function(e) {
            ...
            .then(({ ok, data }) => {
                ...
                // ✅ START: បន្ថែមកូដ១បន្ទាត់នេះ ដើម្បី Update អក្សរលើប៊ូតុង
                // ✅ START: Add this one line of code to update the button's text
