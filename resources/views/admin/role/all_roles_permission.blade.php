...
@section('admin')
                        ...
                        <table class="w-full text-left table-auto min-w-max">
                            <thead>
                                <tr class="bg-slate-50 dark:bg-gray-900">
                                    <th class="sticky top-0 p-4 border-b border-slate-200 bg-slate-50 dark:bg-gray-900"><p class="text-sm dark:text-white font-normal leading-none text-slate-500">{!! __('messages.table_no') !!}</p></th>
                                    <th class="sticky top-0 p-4 border-b border-slate-200 bg-slate-50 dark:bg-gray-900"><p class="dark:text-white text-sm font-normal leading-none text-slate-500">{{ __('messages.roles') }}</p></th>
                                    <th class="sticky top-0 p-4 border-b border-slate-200 bg-slate-50 dark:bg-gray-900"><p class="dark:text-white text-sm font-normal leading-none text-slate-500">{{ __('messages.permission') }}</p></th>
                                    <th class="sticky top-0 p-4 border-b border-slate-200 bg-slate-50 dark:bg-gray-900"><p class="dark:text-white text-sm font-normal leading-none text-slate-500"></p></th>
                                    <th class="sticky top-0 p-4 border-b border-slate-200 bg-slate-50 dark:bg-gray-900"><p class="dark:text-white text-sm font-normal leading-none text-slate-500">{{ __('messages.table_action') }}</p></th>
                                </tr>
                            </thead>
                            <tbody>
                                </tbody>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            function fetchData(pageUrl = "{{ route('search.roles.permission') }}") {
            ...
            $('#search, #perPage').on('keyup change', function() {
                fetchData("{{ route('search.roles.permission') }}?page=1"); // Reset to page 1
            });
