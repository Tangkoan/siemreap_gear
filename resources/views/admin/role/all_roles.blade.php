...
@section('admin')
            ...
            <div class="lg:col-span-full p-0">
                <div class="flex justify-between">
                    <h2 class="text-xl text-default mb-6 flex items-center">
                        ...
                        <div class="px-2">{{ __('messages.roles') }}</div>
                    </h2>
                    <div>
                        <button type="button" class="button-add py-2 px-4 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-transparent focus:outline-hidden disabled:opacity-50 disabled:pointer-events-none">
                            <a href="{{ route('add.roles') }}">{{ __('messages.add_roles') }}</a>
                        </button>
                    </div>
                </div>
                        ...
                        <table class="w-full text-left table-auto min-w-max">
                            <thead>
                                <tr class="bg-slate-50 dark:bg-gray-900">
                                    ...
                                    <th class="sticky top-0 p-4 border-b border-slate-200 bg-slate-50 dark:bg-gray-900"><p class="text-sm font-normal leading-none text-slate-500">{{ __('messages.table_created') }}</p></th>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            function fetchData(pageUrl = "{{ route('search.roles') }}") {
            ...
            $('#search, #perPage').on('keyup change', function() {
                fetchData("{{ route('search.roles') }}?page=1"); // Reset to page 1
            });
