    ...
    <div class="container mx-auto p-6">
        <div class="grid grid-cols-1">
            <div class="card-dynamic-bg rounded-lg shadow-md p-6 transition-all duration-300">
                <h2 class="text-2xl  text-defalut mb-6 flex items-center gap-2">
                ...
                <form id="myForm" method="post" action="{{ route('role.permission.update', $role->id) }}">
                    ...
                    {{-- Role Name --}}
                    <div class="mb-6">
                        <label class="block text-defalut text-sm font-medium mb-2">
                            Role Name:
                            <span class="text-primary text-xl ">{{ $role->name }}</span>
                        </label>
                    </div>
                    ...
                    <div class="mb-6 flex items-center gap-3">
                        <input type="checkbox" id="checkAllPermissions"
                            class="h-5 w-5 rounded  border-primary  text-defalut focus:ring-blue-500  ">
                        <label for="checkAllPermissions" class="text-defalut  text-sm">All
                            Permissions</label>
                    </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                                @foreach ($permissions as $permission)
                                    <label for="perm_{{ $permission->id }}"
                                        class="flex items-center gap-2 text-defalut  text-sm">
                    ...
                    <div class="text-right">
                        <button type="submit"
                            class="bg-primary text-defalut font-bold py-2 px-6 rounded shadow transition-all duration-200">
                            Save
                        </button>
                    </div>
                </form>
