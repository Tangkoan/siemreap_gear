...
@section('admin')
    ...
    <div class="container mx-auto p-6">
        <div class="grid grid-cols-1">
            <div class=" bg-white rounded-lg shadow-md p-6 transition-all duration-300">
                <h2 class="text-2xl font-semibold text-gray-800 mb-6 flex items-center gap-2">
                    ...
                    <a href="{{ route('all.roles.permission') }}" >Edit Roles Permission</a>
                ...
                <form id="myForm" method="post" action="{{ route('role.permission.store') }}">
                    ...
                    <div class="mb-6">
                        <label for="group_name" class="block text-gray-700 text-sm font-medium mb-2">
                            Role Name<span class="text-red-500 text-xl"> {{ $role->name }}</span>
                        </label>
                    ...
                    <div class="mb-6 flex items-center gap-3">
                        ...
                        <label for="all_permission" class="text-gray-700 text-sm">All Permissions</label>
                    </div>
    ...
    $permissions = App\Models\User::getpermissionByGroupName($group->group_name);
                                                                                                                ...
                                                                                                                <fieldset class="mb-6 border border-gray-200 p-4 rounded-md">
                                                                                                                    <legend class="text-sm text-black font-semibold mb-4">{{ $group->group_name }}</legend>

                                                                                                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                                                                                        @foreach($permissions as $permission)
                                                                                                                            <label for="perm_{{ $permission->id }}" class="flex items-center gap-3 text-gray-700 text-sm">
                                                                                                                                <input type="checkbox" name="permission[]" value="" id="customckeck1" {{ App\Models\User::roleHasPermissions($role, $permissions) ? 'checked' : ''}}
                                                                                                                                    class="h-5 w-5 rounded border-gray-400 text-blue-600 focus:ring-blue-500">
                                                                                                                                    {{ $group->group_name }}
                                                                                                                            </label>
                                                                                                                        @endforeach
