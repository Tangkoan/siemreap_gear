...
@section('admin')
    ...
    <button type="button" class="icon-add py-2 px-4 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-transparent text-white focus:outline-hidden  disabled:opacity-50 disabled:pointer-events-none">
    ...
    placeholder="Search for Brand" id="search" name="search" type="text"
    />
    <button
    ...
    <tr>
    <th class="p-4 sticky top-0 bg-slate-50 z-10 border-b border-slate-200">
    ...
    <td class="p-4 py-5">
    <p class="text-sm text-black">{{ $item->brand_name }}</p>
    </td>
    ...
    <button type="button" class="text-gray-500 transition-colors duration-200 dark:hover:text-red-500 dark:text-gray-300 hover:text-red-500 focus:outline-none">
    ...
    $.ajax({
    url: "{{ route('search.brand') }}?page=" + page,
    type: "GET",
    data: {
    search: query,
    perPage: perPage
