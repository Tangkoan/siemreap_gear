...
<div id="detailsModal" class="fixed inset-0 bg-gray-900 bg-opacity-70 backdrop-blur-sm overflow-y-auto h-full w-full hidden z-50 transition-opacity duration-300 opacity-0">
    <div class="relative top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 p-1 border border-primary rounded-lg w-full max-w-3xl shadow-2xl card-dynamic-bg text-defalut scale-95 transition-transform duration-300">
        <div class="mt-3">
            <div class="flex justify-between items-center mb-4 px-5 py-3 border-b border-primary">
                <h3 class="text-2xl " id="modal-title">Transaction Details</h3>
                <button id="closeModal" class="text-defalut transition p-2 rounded-full -mr-2 text-2xl font-bold">&times;</button>
            </div>
            
            <div class="px-5 py-3">
                <div class="overflow-y-auto max-h-[60vh] border border-primary rounded-lg">
                    <table class="min-w-full text-left">
                        <thead class="sticky top-0 card-dynamic-bg text-xs text-defalut uppercase">
                            <tr>
                        ...
                        <tbody id="modal-table-body" class="divide-y divide-primary text-sm">
                            {{-- Details will be loaded here by AJAX --}}
                        </tbody>
                    </table>
                </div>
            </div>
            <div class="items-center px-5 py-4 mt-2 border-t border-primary">
                <button id="closeModalBtn" class="px-6 py-2 bg-primary text-white text-base font-medium rounded-md w-full shadow-sm  transition duration-150 ease-in-out">{{ __('messages.close') }}</button>
            </div>
        </div>
    </div>
</div>
