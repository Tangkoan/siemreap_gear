...
@section('admin')
            ...
            <div class="w-full flex flex-wrap justify-between items-end gap-4 mb-4">
                <div class="flex items-end gap-4">
                    <input type="date" name="date" id="date"
                        ...
                        value="{{ $date }}">
                    <input
                        class="h-10 dark:text-white dark:bg-gray-800 bg-white w-full pr-11 pl-3 placeholder:text-slate-400 border border-slate-200 rounded"
                        placeholder="Search..." id="search" name="search" type="text" />
                </div>
                <div>
                    <a id="exportBtn" href="{{ route('report.orders.export.date', ['date' => $date]) }}"
                        class="h-10 inline-flex items-center px-4 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase hover:bg-green-700">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
            ...
            <div class="w-full bg-white dark:bg-gray-800 shadow-md rounded-lg overflow-hidden">
                <div class="table-wrapper overflow-y-auto max-h-[60vh]">
                    <table class="w-full text-left table-auto min-w-max">
                        ...
                        <tfoot id="report-table-footer" class="bg-slate-50 dark:bg-gray-900"></tfoot>
                    </table>
                </div>
    ...
    {{-- ✅ HTML សម្រាប់ Modal ដែលបានកែសម្រួល --}}
    ...
    <script>
        $(document).ready(function () {
            ...
            function updateExportLink() {
                let date = $('#date').val();
                let search = $('#search').val();
                let baseUrl = "{{ route('report.orders.export.date') }}";
            ...
            // ... Your other functions like fetchData(), updateExportLink()
            fetchData();
        });
    </script>
@endsection