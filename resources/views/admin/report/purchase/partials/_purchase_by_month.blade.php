@php
    $month = date('Y-m');
    $formattedDate = \Carbon\Carbon::parse($month)->format('F Y');
@endphp
...
<div class="w-full flex flex-wrap justify-between items-end gap-4 mb-4">
    <div class="flex items-end gap-4">
        <input type="month" id="month-month" class="h-10 border dark:bg-gray-800 dark:text-white border-slate-300 rounded text-sm w-full" value="{{ $month }}">
        <input class="h-10 dark:text-white dark:bg-gray-800 w-full pr-11 pl-3 placeholder:text-slate-400 border border-slate-200 rounded" placeholder="Search Invoice or Supplier..." id="search-purchase-month" type="text" />
    </div>
    <div>
...
<div class="w-full bg-white dark:bg-gray-800 shadow-md rounded-lg overflow-hidden">
    <div class="table-wrapper overflow-y-auto max-h-[70vh]">
        <table class="w-full text-left table-auto min-w-max">
            ...
            <tfoot id="report-table-footer-month" class="bg-slate-200 dark:bg-slate-900 font-bold"></tfoot>
        </table>
    </div>
</div>