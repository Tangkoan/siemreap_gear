...
$(document).on('click', '#closePurchaseModalBtn', function() {
    $('#purchaseDetailsModal').addClass('hidden');
});

// ✅ Close when clicking outside the modal
$(document).on('click', '#purchaseDetailsModal', function(e) {
    if (e.target.id === 'purchaseDetailsModal') {
        $('#purchaseDetailsModal').addClass('hidden');
    }
});
...
$(document).on('keydown', function(e) {
    if (e.key === "Escape" && !$('#purchaseDetailsModal').hasClass('hidden')) {
        $('#purchaseDetailsModal').addClass('hidden');
    }
});


{{-- ✨ NEW MODERN UI for Order Details Modal --}}
<div id="orderDetailsModal" class="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4">
    <div class="relative w-full max-w-4xl mx-auto">
        <div id="invoice-print-area" class="shadow-2xl rounded-2xl bg-white dark:bg-slate-800 transform transition-all">
            ...
            <div class="px-8 pt-8 pb-4 flex justify-between items-start">
                ...
                <div class="flex items-center gap-2">
                    <button id="printInvoiceBtn" class="p-2 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-full transition-colors" title="Print Invoice">
            ...
            <div class="p-8">
                ...
                <div class="overflow-x-auto rounded-lg border dark:border-slate-700">
                    <table class="min-w-full">
                        ...
                        <tbody id="modal-table-body" class="divide-y divide-slate-200 dark:divide-slate-600 text-sm">
                    ...
                    <div class="w-full max-w-xs space-y-3">
                        ...
                        <div class="border-t border-slate-200 dark:border-slate-700 pt-3 mt-3">
                            <div class="flex justify-between text-slate-600 dark:text-slate-300">
                                ...
                                <span id="summary-payment-method" class="font-semibold"></span>
                            </div>
                        </div>
                    </div>
                </div>
