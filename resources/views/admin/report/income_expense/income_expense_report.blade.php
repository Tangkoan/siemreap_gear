                    ...
                    <div id="filter-container" class="flex items-center gap-4">
                        <div id="daily-filter-group" class="filter-group flex items-center gap-2 hidden">
                            <div class="flex flex-col">
                                ...
                                <input type="date" id="daily_start_date"
                                    class="filter-input-start h-10 border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 rounded-lg text-sm shadow-sm focus:ring-red-500 focus:border-red-500">
                            </div>
                            <div class="flex flex-col">
                                <label for="daily_end_date" class="text-xs mb-1">{{ __('messages.end_date') }}</label>
                                <input type="date" id="daily_end_date"
                                    class="filter-input-end h-10 border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 rounded-lg text-sm shadow-sm focus:ring-red-500 focus:border-red-500">
                            </div>
                        </div>

                        <div id="monthly-filter-group" class="filter-group flex items-center gap-2 hidden">
                            <div class="flex flex-col">
                                <label for="monthly_start_date"
                                    class="text-xs mb-1">{{ __('messages.start_month') }}</label>
                                <input type="month" id="monthly_start_date"
                                    class="filter-input-start h-10 border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 rounded-lg text-sm shadow-sm focus:ring-red-500 focus:border-red-500">
                            </div>
                            <div class="flex flex-col">
                                <label for="monthly_end_date" class="text-xs mb-1">{{ __('messages.end_month') }}</label>
                                <input type="month" id="monthly_end_date"
                                    class="filter-input-end h-10 border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 rounded-lg text-sm shadow-sm focus:ring-red-500 focus:border-red-500">
                            </div>
                        </div>
                        ...
                        <div id="yearly-filter-group" class="filter-group flex items-center gap-2 hidden">
                            <div class="flex flex-col">
                                ...
                                <input type="number" id="yearly_start_date"
                                    class="filter-input-start h-10 border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 rounded-lg text-sm shadow-sm focus:ring-red-500 focus:border-red-500"
                                    placeholder="YYYY" min="2000">
                            </div>
                            <div class="flex flex-col">
                                <label for="yearly_end_date" class="text-xs mb-1">{{ __('messages.end_year') }}</label>
                                <input type="number" id="yearly_end_date"
                                    class="filter-input-end h-10 border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 rounded-lg text-sm shadow-sm focus:ring-red-500 focus:border-red-500"
                                    placeholder="YYYY" min="2000">
                            </div>
                        </div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <button id="export-pdf-btn"
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"
                                ...
                                <polyline points="14 2 14 8 20 8"></polyline>
                                <path
                                    d="M10.29 13.71a2.43 2.43 0 0 1-2.43-2.43 2.43 2.43 0 0 1 2.43-2.43c1.34 0 2.43.95 2.43 2.1 0 .59-.22 1.16-.64 1.57">
                                </path>
                                <path
                                    d="M14.71 13.71a2.43 2.43 0 0 1-2.43-2.43 2.43 2.43 0 0 1 2.43-2.43c1.34 0 2.43.95 2.43 2.1 0 .59-.22 1.16-.64 1.57">
                                </path>
                            </svg>
                            PDF
                        </button>
            ...
            <div id="report-content" style="display: none;">
                <div id="report-summary" class="text-center mb-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div
                            class="flex items-center space-x-4 bg-white dark:bg-slate-800 p-6 rounded-xl shadow-md border-l-4 border-green-500 transform hover:scale-105 transition-transform duration-300">

                            <div class="flex-shrink-0   p-6 rounded-full">
                                {{-- <div class="flex-shrink-0 bg-green-100 dark:bg-green-500/20 p-6 rounded-full"> --}}
                                <img src="{{ asset('images/icons/business-icon.png') }}" class="h-8 w-8"
                                    alt="Revenue Icon">
                            </div>

                            <div>
                                <h3 class="text-lg font-medium text-gray-500 dark:text-gray-400">
                                    {{ __('messages.total_revenue') }}</h3>
                    ...
                    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-md p-4 overflow-hidden">
                        ...
                        <div class="overflow-y-auto max-h-[60vh]">
                            <table class="w-full text-sm text-left">
                                <thead
                                    class="sticky top-0 bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 uppercase text-xs">
                                    <tr>
                                ...
                                <tbody id="income-details-body" class="divide-y divide-gray-200 dark:divide-slate-700">
                                </tbody>
                            </table>
                        </div>
                    </div>
                    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-md p-4 overflow-hidden">
                        ...
                        <div class="overflow-y-auto max-h-[60vh]">
                            <table class="w-full text-sm text-left">
                                ...
                                <tbody id="expense-details-body" class="divide-y divide-gray-200 dark:divide-slate-700">
                                </tbody>
                            </table>
                        </div>
                    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            function switchTab(type) {
                ...
                $(`.report-tab[data-type="${type}"]`).removeClass('text-gray-600 dark:text-gray-300').addClass(
