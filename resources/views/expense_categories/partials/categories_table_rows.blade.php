{{-- នេះជាកូដសម្រាប់តែ <tbody> ប៉ុណ្ណោះ --}}

@forelse($categories as $key => $item)
    <tr class="text-defalut">
        <td class="p-4 border-b border-slate-200">
            <p class="font-normal">{{ $categories->firstItem() + $key }}</p>
        </td>
        <td class="p-4 border-b border-slate-200">
        ...
        <td class="p-4 border-b border-slate-200">
            {{-- នេះជា Column ថ្មីដែលយើងបានបន្ថែម (withCount) --}}
            <span class="inline-flex items-center rounded-md bg-blue-50 px-2 py-1 text-xs font-medium text-blue-700 ring-1 ring-inset ring-blue-700/10 dark:bg-blue-800 dark:text-blue-200">
        ...
        <td class="p-4 border-b border-slate-200">
            <div class="flex gap-2">
                @can('expense.category.edit')
                    ...
                    {{-- កំហុសទី១ ត្រូវបានកែនៅទីនេះ (type="button" ត្រូវនៅបន្ទាត់តែមួយ) --}}
                    <button type="button" 
                            ...
                            title="Edit">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="http://www.w3.org/2000/svg" stroke-width="1.5" stroke="currentColor" class="size-5">
...
{{-- កំហុសទី២ ត្រូវបានកែនៅទីនេះ (ត្រូវតែ @endforelse) --}}
@endforelse