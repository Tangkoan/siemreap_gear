...
@forelse($expenses as $key => $item)
    <tr class="text-defalut">
        <td class="p-4 border-b border-slate-200">
            <p class="font-normal">{{ $expenses->firstItem() + $key }}</p>
        </td>
        <td class="p-4 border-b border-slate-200">
            <p class="font-normal">{{ \Carbon\Carbon::parse($item->expense_date)->format('d-M-Y') }}</p>
        </td>
        <td class="p-4 border-b border-slate-200">
            {{-- បង្ហាញ Category name. '??' គឺសម្រាប់ការពារ error បើ category ត្រូវបានលុប --}}
            <p class="font-normal">{{ $item->category->name ?? 'N/A' }}</p>
        </td>
        <td class="p-4 border-b border-slate-200">
            <p class="font-normal">{{ $item->description }}</p>
        </td>
        <td class="p-4 border-b border-slate-200">
            <p class="font-bold text-red-600">${{ number_format($item->amount, 2) }}</p>
        </td>
        <td class="p-4 border-b border-slate-200">
            {{-- បង្ហាញឈ្មោះអ្នកកត់ត្រា --}}
            <p class="font-normal">{{ $item->user->name ?? 'N/A' }}</p>
        </td>
        <td class="p-4 border-b border-slate-200">
