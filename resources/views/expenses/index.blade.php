...
<style>
    ...
    /* Style សម្រាប់ Modal (លាក់/បង្ហាញ) */
    .modal-hidden {
        display: none;
    }
</style>
        ...
        <div class="lg:col-span-full p-0">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
                <h2 class="text-xl text-defalut flex items-center">
                    {{-- Icon ថ្មីសម្រាប់ Expense --}}
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 18.75a60.07 60.07 0 0 1 15.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 0 1 3 6h-.75m0 0v-.75A.75.75 0 0 1 3 4.5h.75m0 0h.75A.75.75 0 0 1 4.5 6v.75m0 0v-.75A.75.75 0 0 1 3.75 4.5h.75m8.25 0h.75a.75.75 0 0 1 .75.75v.75m0 0v-.75a.75.75 0 0 1-.75-.75h-.75m0 0h.75A.75.75 0 0 1 13.5 6v.75m0 0v-.75a.75.75 0 0 1-.75-.75h-.75m5.25 0h.75a.75.75 0 0 1 .75.75v.75m0 0v-.75a.75.75 0 0 1-.75-.75h-.75m0 0h.75a.75.75 0 0 1 .75.75v.75m0 0v-.75a.75.75 0 0 1-.75-.75h-.75M3 13.5h18M3 17.25h18" />
                    </svg>
                    {{-- កែ Title --}}
                    <div class="px-2 text-3xl font-bold text-defalut">{{ __('messages.expense') }}</div> 
                </h2>
                ...
                <div class="flex items-center gap-x-2">
                    ...
                    @can('expense.add')
                    <button type="button" id="addExpenseBtn" class="bg-primary text-white py-2 px-4 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-transparent focus:outline-hidden">
                ...
                <div class="w-full flex justify-between items-center mb-3 mt-1 pl-3">
                    ...
                    <div class="ml-3">
                        <div class="w-72 relative">
                            <div class="relative">
                                <input class="text-defalut card-dynamic-bg w-full pr-11 h-10 pl-3 py-2 text-sm border border-primary rounded transition duration-200 ease focus:outline-none focus:border-slate-400 hover:border-slate-400 shadow-sm focus:shadow-md" placeholder="ស្វែងរកតាមការពិពណ៌នា..." id="search" name="search" type="text" />
                    ...
                    <table class="w-full text-left table-auto min-w-max ">
                        <thead>
                            <tr>
                                ...
                                <th class="sticky top-0 p-4 border-b border-slate-200 "><p class="text-sm font-normal leading-none text-primary">ថ្ងៃខែចំណាយ</p></th>
                                <th class="sticky top-0 p-4 border-b border-slate-200 "><p class="text-sm font-normal leading-none text-primary">ប្រភេទ</p></th>
                                ...
                                <th class="sticky top-0 p-4 border-b border-slate-200 "><p class="text-sm font-normal leading-none text-primary">ទឹកប្រាក់</p></th>
                                <th class="sticky top-0 p-4 border-b border-slate-200 "><p class="text-sm font-normal leading-none text-primary">អ្នកកត់ត្រា</p></th>
...
{{-- 5. ADD/EDIT MODAL --}}
<div id="expenseModal" class="fixed z-50 inset-0 overflow-y-auto modal-hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                ...
                <div class="card-dynamic-bg px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <span id="error_expense_category_id" class="text-red-500 text-sm"></span>
                        </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        // --- 1. កំណត់ค่า TOAST (ដូចវីដេអូ Add Product របស់អ្នក) ---
        ...
        // --- ផ្នែកទី១: ផ្ទុកទិន្នន័យ (Search, PerPage, Pagination) ---

        function fetchData(pageUrl = "{{ route('search.expense') }}") {
        ...
        fetchData(); // ដំណើរការពេលបើកទំព័រ

        $('#search, #perPage').on('keyup change', function() {
            fetchData("{{ route('search.expense') }}?page=1"); // Reset to page 1
        });
        ...
        const modal = $('#expenseModal'); // ត្រូវប្រាកដថា ID នេះត្រូវ
        const expenseForm = $('#expenseForm'); // ត្រូវប្រាកដថា ID នេះត្រូវ
        ...
        // Submit Form (សម្រាប់ cả Add និង Edit)
        expenseForm.on('submit', function(e) {
            e.preventDefault();
            $('#saveBtn').prop('disabled', true).text('កំពុងរក្សាទុក...');
            
            let expenseId = $('#expenseId').val();
            let url = expenseId ? "{{ url('api/expenses') }}/" + expenseId : "{{ route('expenses.json.store') }}";
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    // --- 2. កែនៅទីនេះ ---
                    // ប្រើ Toast.fire ដូចវីដេអូ Add Product
                    Toast.fire({
                        icon: 'success',
                        title: response.message // 'Expense created successfully!'
                    });
                    // --- ចប់ការកែ ---
                },
                error: function(xhr) {
                    if (xhr.status === 422) {
                        ...
                        Toast.fire({ icon: 'error', title: 'មានបញ្ហាកើតឡើង' });
                    }
                },
                complete: function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        error: function(xhr) {
                            Toast.fire({ icon: 'error', title: 'មិនអាចលុបបានទេ' });
                        }
                    });
                }
            });
