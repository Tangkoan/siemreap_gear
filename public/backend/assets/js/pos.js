document.addEventListener('DOMContentLoaded', () => {
      ...
      function calculateTotals() {
        const subtotalSum = cartItems.reduce((sum, item) => sum + item.price * item.qty, 0);
      ...
      function updateCart() {
        ...
        cartItems.forEach((item, index) => {
          ...
          removeBtn.className = 'bg-red-500 text-white px-2 py-2 mt-2 rounded text-xs';
          removeBtn.onclick = () => {
        ...
        totalPayableDiv.textContent = `Total Payable : ${formatCurrency(total)}`;
      }
      ...
      document.getElementById('payNowBtn').addEventListener('click', () => {
        ...
        modalDiscount.textContent = formatCurrency(parseFloat(discountInput.value) || 0);
        modalShipping.textContent = formatCurrency(parseFloat(shippingInput.value) || 0);
      ...
      receivedAmountInput.addEventListener('input', () => {
        ...
        const totalAmount = parseFloat(modalTotalPayable.textContent.replace('$', '').trim()) || 0;
      ...
      previewInvoiceBtn.onclick = () => {
        if (cartItems.length === 0) {
          alert('The cart is empty. Please add items to preview the invoice.');
          return;
        }
      ...
      function generateInvoice(customerName, validity, phone) {
        ...
        const { taxAmount, discount, shipping, subtotalSum, total } = calculateTotals();
      ...
      function createInvoice(container, cartItems, taxPercent = 0, discountVal = 0, shippingVal = 0, customerName = '', validity = '', phone = '') {
  ...
  div.innerHTML = `
    ...
    <div class="flex flex-col md:flex-row justify-between mb-4 p-4 print:p-0">
      ...
      <div class="w-full md:w-1/2 bg-gray-50 p-2 rounded shadow-sm md:ml-2">
        ...
        <p><span class="font-semibold">Customer Name:</span> ${customerName || '____________________'}</p>
        <p><span class="font-semibold">Validity:</span> ${validity || '______________________________'}</p>
        <p><span class="font-semibold">Phone:</span> ${phone || '_______________________________'}</p>
      </div>
    </div>
        ...
        <tbody>
          ${cartItems.map((item, i) => `
            <tr class="hover:bg-gray-100 transition-colors duration-300">
              ...
              <td class="border border-gray-300 px-2 py-1 text-center">${formatCurrency(item.price)}</td>
              <td class="border border-gray-300 px-2 py-1 text-center">${item.qty}</td>
              <td class="border border-gray-300 px-2 py-1 text-center">${formatCurrency(item.price * item.qty)}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
    ...
    <div class="flex flex-col md:flex-row mb-4 gap-4 text-xs p-4 print:p-0">
      ...
      <div class="md:w-1/2 flex justify-end">
        <div class="w-48 bg-gray-50 p-2 rounded shadow-sm space-y-2">
          <div class="flex justify-between"><span class="font-semibold">Subtotal:</span><span>${formatCurrency(subtotalSum)}</span></div>
          <div class="flex justify-between"><span class="font-semibold">Tax (${taxPercent}%):</span><span>${formatCurrency(taxAmount)}</span></div>
          <div class="flex justify-between"><span class="font-semibold">Shipping:</span><span>${formatCurrency(shippingVal)}</span></div>
          <div class="flex justify-between"><span class="font-semibold">Discount:</span><span>${formatCurrency(discountVal)}</span></div>
          <div class="flex justify-between"><span class="font-semibold">Total:</span><span>${formatCurrency(totalDisplay)}</span></div>
        </div>
      </div>
    </div>
